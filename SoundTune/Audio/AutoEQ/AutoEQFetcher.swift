// SoundTune/Audio/AutoEQ/AutoEQFetcher.swift
import Foundation
import os
...
final class AutoEQFetcher {
    ...
    private let logger = Logger(subsystem: "com.soundtune.SoundTune", category: "AutoEQFetcher")

    // MARK: - URLs

    private static let indexURL = URL(string: "https://soundtune.app/INDEX.md")!
    private static let profileBaseURL = "https://soundtune.app/profiles/"

    // MARK: - Cache Paths

    private static var cacheDirectory: URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
            .appendingPathComponent("SoundTune")
    }
    ...
    func refreshCatalogFromGitHub() async {
        // Network calls are disabled for air-gapped environments.
        if catalog.isEmpty {
            catalogState = .error("Network access is disabled")
        } else {
            catalogState = .loaded
        }
        logger.info("Network access is disabled: catalog refresh skipped")
    }
    ...
    func fetchProfile(for entry: AutoEQCatalogEntry) async throws -> AutoEQProfile {
        ...
        // Network calls are disabled for air-gapped environments.
        logger.warning("Attempted to fetch profile in offline mode: \(entry.name)")
