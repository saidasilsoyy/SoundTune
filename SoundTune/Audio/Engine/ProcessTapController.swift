// SoundTune/Audio/Engine/ProcessTapController.swift
import AudioToolbox
import Foundation
import os
...
final class ProcessTapController: ProcessTapControlling {
    ...
    init(
        ...
        self.logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "SoundTune", category: "ProcessTapController(\(app.name))")
    }
    ...
    private func createSecondaryTap(for outputUIDs: [String]) throws {
        ...
        let description = buildAggregateDescription(
            ...
            tapUUID: tapDesc.uuid,
            name: "SoundTune-\(app.id)-secondary"
        )
    ...
    private func performDeviceSwitch(to outputUIDs: [String]) throws {
        ...
        if let deviceSampleRate = try? primaryResources.aggregateDeviceID.readNominalSampleRate() {
            ...
            deviceEQProcessor?.updateSampleRate(deviceSampleRate)
    ...
    nonisolated static func processMappedBuffers(
        ...
        for outputIndex in 0..<outputBufferCount {
            ...
            if let deviceEQProc, deviceEQProc.isEnabled, eqCanProcessStereoInterleaved {
                deviceEQProc.process(input: outputSamples, output: outputSamples, frameCount: frameCount)
            }
