// SoundTune/Audio/Engine/OrphanedTapCleanup.swift
import AudioToolbox
import os

private let logger = Logger(subsystem: "com.soundtune.SoundTune", category: "OrphanedTapCleanup")

/// Scans CoreAudio for orphaned SoundTune aggregate devices and destroys them.
/// Orphans occur when SoundTune crashes or is force-killed (`kill -9`), leaving
/// aggregate devices with `.mutedWhenTapped` process taps that silently mute apps.
enum OrphanedTapCleanup {
    /// Destroys any aggregate devices named "SoundTune-*" left over from a previous session.
    ...
    static func destroyOrphanedDevices() {
        ...
        for device in devices {
            ...
            guard let name = try? device.readDeviceName(),
                  name.hasPrefix("SoundTune-") else { continue }
        ...
        if destroyedCount == 0 {
            logger.info("[CLEANUP] No orphaned SoundTune devices found")
        } else {
