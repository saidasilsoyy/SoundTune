// SoundTune/Settings/SettingsManager.swift
import Foundation
import os
...
final class SettingsManager {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "SoundTune", category: "SettingsManager")
    ...
    struct Settings: Codable {
        ...
        var deviceEQSettings: [String: EQSettings] = [:]  // deviceUID → EQ settings
        ...
        init(from decoder: Decoder) throws {
            ...
            if !decodedAppSettings.defaultNewAppVolume.isFinite || decodedAppSettings.defaultNewAppVolume < 0 {
                ...
                decodedAppSettings.defaultNewAppVolume = min(decodedAppSettings.defaultNewAppVolume, 1.0)
            }
    ...
    init(directory: URL? = nil) {
        let baseDir = directory ?? FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!.appendingPathComponent("SoundTune")
    ...
    func setVolume(for identifier: String, to volume: Float) {
        settings.appVolumes[identifier] = normalizedDeviceVolume(volume)
        scheduleSave()
    }
    ...
    func ignoreApp(_ identifier: String, info: IgnoredAppInfo) {
        ...
        settings.pinnedApps.remove(identifier)
        settings.pinnedAppInfo.removeValue(forKey: identifier)
        // Clear per-app settings — SoundTune won't interact with this app
    ...
    func updateAppSettings(_ newSettings: AppSettings) {
        ...
        sanitizedSettings.defaultNewAppVolume = normalizedDeviceVolume(newSettings.defaultNewAppVolume)

        // Handle launch at login separately via ServiceManagement. If macOS rejects
        // the change, keep persisted state aligned with the system rather than
        // showing a successful toggle that did not actually apply.
        if sanitizedSettings.launchAtLogin != settings.appSettings.launchAtLogin {
            let didApply = setLaunchAtLogin(sanitizedSettings.launchAtLogin)
            if !didApply {
                sanitizedSettings.launchAtLogin = settings.appSettings.launchAtLogin
            }
        }
        settings.appSettings = sanitizedSettings
        LanguageManager.shared.currentLanguage = sanitizedSettings.language
        scheduleSave()
    }
    ...
    func resetAllSettings() {
        ...
        LanguageManager.shared.currentLanguage = settings.appSettings.language
