// SoundTune/Views/Components/DevicePicker.swift
import SwiftUI
...
struct DevicePicker: View {
    ...
    enum MenuItem: Identifiable, Equatable {
        ...
        var name: String {
            switch self {
            case .systemAudio: return t("System Audio")
    ...
    private var itemText: some View {
        ...
        case .systemAudio:
            VStack(alignment: .leading, spacing: 1) {
                Text(t("System Audio"))
                if isDisabled {
                    Text(t("Not available in multi mode"))
                        .font(DesignTokens.Typography.caption)
                        .foregroundStyle(DesignTokens.Colors.textQuaternary)
                } else {
                    Text(t("Follows macOS default"))
                        .font(DesignTokens.Typography.caption)
                        .foregroundStyle(DesignTokens.Colors.textTertiary)
                }
            }
...
extension DevicePicker {
    static func routingSubtitle(
        ...
        case .multi:
            ...
            switch valid.count {
            case 0:  return t("Multi")
            case 1:  return "\(t("Multi")) · \(valid[0].name)"
            default: return "\(t("Multi")) · \(valid.count) \(t("devices"))"
            }
        }
    }
}
